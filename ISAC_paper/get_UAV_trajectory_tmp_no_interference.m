function uav = get_UAV_trajectory_tmp_no_interference(uav_t, W_opt, R_opt, N, num_user, num_target, noise_power, user, uav_z, target, sensing_th, V_max, delta_t, PARAM)
    ...
    for n = 1 : PARAM.N
        for k = 1 : PARAM.NUM_USER
            ...
            channel(:, k, n) = get_channel_tmp(uav_t(n, :), PARAM.USER(k,:), PARAM.SCALING, PARAM.UAV_Z, PARAM.NUM_ANTENNA);
    ...
    cvx_begin
        ...
        for n = 1 : N
            ...
            for k = 1 : num_user
                ...
                first_val = log2((E(k,n) + F(k,n)) / distance_user(k,n) + PARAM.NOISE_POWER_SCALING);
                first_dev_tmp1 = -log2(exp(1)) * (E(k,n) + F(k,n)) / distance_user(k,n)^2;
                first_dev_tmp2 = (E(k,n) + F(k,n)) / distance_user(k,n) + PARAM.NOISE_POWER_SCALING;
                ...
                subject to
                    100 * PARAM.SCALING * (1 / exp(eta(k,n))) <= 100 *PARAM.SCALING * (norm([uav_t(n,1) - user(k,1), uav_t(n,2) - user(k,2)])^2 + sum(2 * (uav_t(n,:) - user(k,:)) .* (uav(n,:) - uav_t(n,:))));
            end
