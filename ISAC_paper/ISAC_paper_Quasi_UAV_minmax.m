function sum_rate_final = ISAC_paper_Quasi_UAV()
    ...
    for episode = 1 : PARAM.NUM_EPISODE
        ...
        while(1)
            ...
            uav = get_UAV_position_minmax(uav_t, W_opt, R_opt, PARAM.USER, PARAM.NUM_USER, PARAM.CHANNEL_GAIN, PARAM.NOISE_POWER, PARAM.SENSING_TH, PARAM.NUM_TARGET, PARAM.TARGET, trust_region, PARAM.UAV_Z);
            ...
            if min(user_rate_current_UAV) > min(user_rate_prev_UAV)
                uav_t = uav;
                trust_region = PARAM.TRUST_REGION;
