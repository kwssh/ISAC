function sum_rate_final = ISAC_paper_Mobile_UAV_minmax()
    clear
    format long
    %-----------------------------setting parameter-----------------------------------------------------------------------------------------------------------------------------%
    PARAM.SCALING = 1000;
    ...
    for episode = 1 : PARAM.NUM_EPISODE
        ...
        if episode == 1
            ...
            [uav_t, W_t, R_t] = get_init_trajectory_SF_min_max(PARAM.TARGET, PARAM.NUM_ANTENNA, PARAM.NUM_USER, PARAM.NUM_TARGET, PARAM.SENSING_TH_SCALING, PARAM.P_MAX, PARAM.SCALING, PARAM.V_MAX, PARAM.N, PARAM.UAV_START(1), PARAM.UAV_END(1), PARAM.UAV_START(2), PARAM.UAV_Z);
            %----------------------------------------------------------------------------------------------------------------------------------------------------------------------------%
        ...
        cvx_begin
            ...
            expressions tau_tmp(PARAM.NUM_USER)
            ...
            for n = 1 :PARAM.N
                ...
                for k = 1 : PARAM.NUM_USER
                    ...
                    tau_tmp(k) = tau_tmp(k) + sum_rate(k,n);
                end
        ...
        while(1)
            ...
            uav = get_UAV_trajectory_min_max(uav_t, W_opt, R_opt, PARAM.USER, PARAM.NUM_USER, PARAM.CHANNEL_GAIN, PARAM.NOISE_POWER, PARAM.SENSING_TH, PARAM.NUM_TARGET, PARAM.TARGET, trust_region, PARAM.V_MAX, PARAM.N, PARAM.UAV_Z, PARAM.DELTA_T);
    ...
    % get_received_BEAM_GAIN(W_opt(:,:,:,1), R_opt(:,:,1), PARAM.NUM_USER, PARAM.NUM_ANTENNA, PARAM.SENSING_TH_SCALING, PARAM.SCALING, distance_user_t, distance_target_t, PARAM.UAV_Z);
end