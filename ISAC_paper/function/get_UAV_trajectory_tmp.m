function uav = get_UAV_trajectory_tmp(uav_t, W_opt, R_opt, N, num_user, num_target, noise_power, user, uav_z, target, sensing_th, V_max, delta_t, PARAM)
    ...
    for n = 1 : PARAM.N
        for k = 1 : PARAM.NUM_USER
            ...
            channel(:, k, n) = get_channel_tmp(uav_t(n, :), PARAM.USER(k,:), PARAM.SCALING_TMP, PARAM.UAV_Z, PARAM.NUM_ANTENNA);
        ...
        for j = 1 : PARAM.NUM_TARGET
            ...
            steering_target(:, j, n) = get_steering(distance_target_t_tmp(j, n), PARAM.SCALING_TMP, PARAM.UAV_Z, PARAM.NUM_ANTENNA);
