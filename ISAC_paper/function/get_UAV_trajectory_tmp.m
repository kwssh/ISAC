function uav = get_UAV_trajectory_tmp(uav_t, W_opt, R_opt, N, num_user, num_target, noise_power, user, uav_z, target, sensing_th, V_max, delta_t, PARAM)
    ...
    for n = 1 : PARAM.N
        for k = 1 : PARAM.NUM_USER
            ...
            channel(:, k, n) = get_channel_tmp(uav_t(n, :), PARAM.USER(k,:), PARAM.SCALING * 100, PARAM.UAV_Z, PARAM.NUM_ANTENNA);
        ...
        for j = 1 : PARAM.NUM_TARGET
            ...
            steering_target(:, j, n) = get_steering(distance_target_t_tmp(j, n), PARAM.SCALING, PARAM.UAV_Z, PARAM.NUM_ANTENNA);
    ...
    cvx_begin
        ...
        for n = 1 : N
            ...
            for k = 1 : num_user
                ...
                first_val = log2((E(k,n) + F(k,n)) / distance_user(k,n) + PARAM.NOISE_POWER_SCALING * 100 * 100);
                first_dev_tmp1 = -log2(exp(1)) * (E(k,n) + F(k,n)) / distance_user(k,n)^2;
                first_dev_tmp2 = (E(k,n) + F(k,n)) / distance_user(k,n) + PARAM.NOISE_POWER_SCALING * 100 * 100;
                first_dev = first_dev_tmp1 / first_dev_tmp2;

                gamma_low(k,n) = first_val + first_dev * (distance_user_uav(k,n) - distance_user(k,n));

                gamma_tilda(k,n) = gamma_low(k,n) - (log_sum_exp([log(F(k,n)) + eta(k,n) log(PARAM.NOISE_POWER_SCALING * 100 * 100)])) / log(2);

                % gamma_tilda(k,n) = gamma_low(k,n) - log2(PARAM.NOISE_POWER_SCALING);
                
                subject to
                    100 * PARAM.SCALING * (1 / exp(eta(k,n))) <= 100 *PARAM.SCALING * (PARAM.UAV_Z^2 + norm([uav_t(n,1) - user(k,1), uav_t(n,2) - user(k,2)])^2 + sum(2 * (uav_t(n,:) - user(k,:)) .* (uav(n,:) - uav_t(n,:))));
            end
