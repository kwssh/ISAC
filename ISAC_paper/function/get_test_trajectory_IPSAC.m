function [rate, error] = get_test_trajectory_IPSAC(W, R, p_max, sensing_th, num_target, channel, channel_her, noise_power, steering_target, steering_target_her, distance_target, N, c)
    ...
    for n = 1:N
        ...
        for k = 1:num_user
            ...
            objective_1 = objective_1_tmp + real(trace(channel(:,k,n) * channel_her(k,:,n) * (c_total * R(:,:,n)))) + noise_power;
            objective_1 = log2(objective_1);
    
            objective_2 = objective_2_tmp + real(trace(channel(:,k,n) * channel_her(k,:,n) * (c_total * R(:,:,n)))) + noise_power;
