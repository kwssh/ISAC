function sum_rate_final = ISAC_paper_Mobile_UAV_comm()
    ...
    for episode = 1:num_episdoe
        ...
        if episode == 1
            ...
            % uav_t = [450 525; 492.5988371493868 517.9211674059339; 550 525];
            % uav_t = [500 525];
        ...
        for n = 1:N
            ...
            cvx_begin
                ...
                variable R(num_antenna, num_antenna, N) complex
                ...
                for n = 1: N
                    ...
                    for k = 1:num_user
                        ...
                        tmp(k,n) = objective_1_tmp(k,n) + real(trace(channel_t(:,k,n) * channel_her_t(k,:,n) * R(:,:,n))) + noise_power;
                    ...
                    subject to
                        ...
                        R(:,:,n) == hermitian_semidefinite(num_antenna);
...
%-------------------------------------------------------------------------------------------------------------------------------------------------------%
        ...
        while(1)
            ...
            uav = get_UAV_trajectory_no(uav_t, W_opt, R_opt, user, num_user, channel_gain, noise_power / scaling^2, sensing_th / scaling^2, num_target, target, trust_region, V_max, N);
