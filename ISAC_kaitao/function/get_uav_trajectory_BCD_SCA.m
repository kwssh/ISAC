function [uav, user_rate] = get_uav_trajectory_BCD_SCA(distance_user, distance_target, num_user, num_target, N, gamma_0, p_max, num_antenna, sensing_th, PARAM, uav_t, V_max, delta_t, A_opt, E_opt, rate_th, isac_duration, episilon_sca)
    ...
    % for episode_BCD = 1 : num_episode_BCD
        ...
        % [uav, z_user, user_rate]= get_trajectory_user_SCA(distance_user_l, distance_target_l, num_user, num_target, N, gamma_0, p_max, num_antenna, sensing_th, PARAM, uav_t, V_max, delta_t, A_opt, E_opt, rate_th, isac_duration, episilon_sca);
        [uav, z_user, user_rate]= asdf(distance_user_l, distance_target_l, num_user, num_target, N, gamma_0, p_max, num_antenna, sensing_th, PARAM, uav_t, V_max, delta_t, A_opt, E_opt, rate_th, isac_duration, episilon_sca);
