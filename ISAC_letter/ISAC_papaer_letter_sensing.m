...
sensing_threshold = 10^(-2) * scaling^2;
pho = 0.35;
...
V_l = get_init_letter_qwer(num_user, num_antenna, num_target, channel, channel_her, sinr_threshold, noise_power, sterring_target, sterring_target_her, sensing_threshold, p_max, p_circuit, pho);
...
for i = 2:num_episode
    ...
    cvx_begin
        ...
        sensing_power_obj = get_sensing_power_const_letter(sensing_power, num_target, sterring_target, sterring_target_her, V_total, sensing_threshold);
        sensing_power_obj = sensing_power_obj + sensing_threshold;

        maximize(sum(sensing_power_obj))
